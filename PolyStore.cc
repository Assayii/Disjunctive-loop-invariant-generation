...
void PolyStore::extract_linear_part( MatrixStore & m) const {
   ...
   for (vi=cs.begin(); vi!=cs.end();vi++){
      ...
      if (cc.is_equality()){
         ...
         // Convert the constraint cc into a linear expression to be added into
         // the matrix store

         // conversion to a SparseLinear_Expression starts here..

         // Post-Comments: TODO-- why cannot I add a constructor SparseLinExpr(Linear_Expression)
         // Post-post-comments: Should keep SparseLinExpr PPL free?
         
         // iterate through coefficients
         for ( i=0;i<n;i++){
            Coefficient t = cc.coefficient(Variable(i));
            if (!t.fits_sint_p()){
               cout<<"Error in PolyStore::extract_linear_part-- gmp integer overflow"<<endl;
               exit(1);
            }
...
PolyStore * PolyStore::clone() const{
   ...
   Constraint_System cs= p->minimized_constraints(); // extract the constraints from the polyhedron
