...
void Context::initialize(var_info *f, var_info * fd, var_info * fm, MatrixStore *ms, PolyStore * ps, DisequalityStore * ds,
...
Context::Context(var_info * f, var_info * fd, var_info * fm, MatrixStore * ms, PolyStore * ps, DisequalityStore *ds,
          vector<Expression> * eqs, vector<Expression> * ineqs){
...
void Context::add_to_matrix_store(Linear_Expression lin){
   ...
   for (i=0;i<nd;i++){
      l.set_coefficient(i,
                        handle_integers(lin.coefficient(Variable(i)))
                        );
...
void Context::check_consistent(){
   incon= !ms->is_consistent() || !ps->is_consistent() || !ds->is_consistent();
}
...
bool Context::move_constraints_equalities(){
   // Check eqs for constraints that are linear equalities or linear transforms and
   // In case a linear equality is obtained then transfer it to the equality and inequality stores
   // In case a linear transform is obtained, then apply the transform on all the expressions and
   //   Add it to the disequality store
   // return true if some action occured and false otherwise
   cout<<endl<<"In move_constraints_equalities(): "<<endl;
   vector<Expression>::iterator vi;
   bool some=false;
   for(vi=eqs->begin();vi<eqs->end();vi++){
      cout<<endl<<"Expression is: "<<(*vi)<<endl;
      if ((*vi).is_pure_a()){
         cout<<endl<<"In (*vi).is_pure_a(): "<<endl;
         cout<<endl<<"(*vi).convert_linear() is: "<<(*vi).convert_linear()<<endl;
         add_linear_equality((*vi).convert_linear());
         some=true;
      } else if ((*vi).is_pure_b()){
         cout<<endl<<"In (*vi).is_pure_b(): "<<endl;
         cout<<endl<<"(*vi).convert_transform() is: "<<(*vi).convert_transform()<<endl;
...
bool Context::move_constraints_inequalities(){
   ...
   cout<<endl<<"In move_constraints_inequalities(): "<<endl;
   vector<Expression>::iterator vi;
   bool some=false;
   for(vi=ineqs->begin();vi<ineqs->end();vi++){
      if ((*vi).is_pure_a()){
...
bool  Context::move_constraints(){
   cout<<endl<<"In move_constraints()"<<endl;
...
void Context::simplify_inequalities(){
   ...
   // Are there some rules that can be used to simplify the expressions using the
   // polyhedral store?
...
void Context::simplify_repeat(){
   cout<<endl<<"In simplify_repeat(): "<<endl;
   bool some=true;
   while (some){
...
Expression & Context::choose_maximal_factor_equalities(){
   ...
   if (factors->empty()){
      cerr<<" In Context::choose_maximal_factor_equalities()...."<<endl<<endl;
      cerr<<"Fatal Error: Asked to choose a maximal factor on an empty factors vector"<<endl;
...
bool Context::split_on_factor_equalities(LinTransform & lt){
   cout<<endl<<"In split_on_factor_equlities(LinTransform & lt): "<<endl;
   vector<Expression>::iterator vi;
   bool split=false;
   if (!is_viable_equalities(lt)){
      ...
      split=true;
      DisequalityStore * ds1=ds->clone();
      child1=new Context(f,fd,fm,ms->clone(),ps->clone(),ds1); // create a new context by cloning the appropriate stores
...
int Context::factorizing_strategy_equalities(){
   cout<<endl<<"In factorizing_strategy_equalities(): "<<endl;
...
void Context::recursive_strategy(vector<Location *> * loclist, C_Polyhedron * dualp, int wtime, bool timed ){
   Timer one_timer;
   recursive_strategy(loclist,dualp,wtime,timed,one_timer);
}

void Context::recursive_strategy(vector<Location *> * loclist, C_Polyhedron * dualp, int wtime, bool timed, Timer & one_timer ){
   cout<<"In recursive_strategy..."<<endl;
   int i=1;
   ...
   while (i>0){
      if (ps->contained(dualp)){
         cout<<endl<<"prune_count++"<<endl;
         prune_count++;
         cout<<endl<<"ps is "<<(*ps)<<endl;
         cout<<endl<<"dualp is "<<(*dualp)<<endl;
         return;
      }
      i= factorizing_strategy_equalities();
   ...
   cout<<"The recursive_strategy End."<<endl;
}
...
void Context::split_01_strategy(vector <Context *>* children){
   // choose an unresolved multiplier and create two children by instantiating with 0 and 1
...
void Context::split_01_strategy(Clump & vp){
   ...
   // now go though all the multipliers for which zero or one is forbidden and apply the remaining
   // choose 0/1 values for the multiplier and expand
   LinTransform lt(r,fm);
   Context * child1;
...
void Context::split_01_strategy(vector<Location *> * loclist, C_Polyhedron * dualp, int wtime, bool timed, Timer & one_timer){
   cout<<endl<<"In split_01_strategy..."<<endl;
   // choose an unresolved multiplier and create two children by instantiating with 0 and 1
   // as long as these instantiations are allowed
   ...
   if (index== NO_UNRESOLVED_MULTIPLIER){
      ...
      for (vi=loclist->begin();vi<loclist->end();vi++){
         cout<<"vi-loc: "<<endl<<*(*vi)<<endl;
         cout<<"inv: "<<endl;(*vi)->extract_invariants_and_update(ps->get_poly_reference(), *dualp);
         cout<<"vi-loc: "<<endl<<*(*vi)<<endl;
      }
      cout<<"The first 'extract_invariants_and_update' printed over."<<endl;
      return; // nothing to be done
   }
   ...
   LinTransform lt(r,fm);
   Context * child1;
   cout<<endl<<"LinTransform lt(r,fm) is: "<<lt<<endl;
   cout<<endl<<"r is: "<<r<<endl<<"fm is: "<<fm<<endl;
   
   for(i=0;i<r;i++){
      cout<<endl<<"i is: "<<i<<endl;
      cout<<endl<<"tt[i] is: "<<tt[i]<<endl;
      cout<<endl<<"lt[i] is: "<<lt[i]<<endl;
      switch(tt[i]){
         case ZERO_ONE_FORBIDDEN:
            continue;
         case ZERO_FORBIDDEN:
   ...
   if (index== NO_UNRESOLVED_MULTIPLIER){
      ...
      vector<Location *>::iterator vi;
      for (vi=loclist->begin();vi<loclist->end();vi++){
         (*vi)->extract_invariants_and_update(ps->get_poly_reference(), *dualp);
      }
      cout<<"The second 'extract_invariants_and_update' printed over."<<endl;
      return; // nothing to be done
   }
   ...
   cout<<"The split_01_strategy End."<<endl;
   return;
...
void Context::collect_generators(Generator_System & g){
   ...
   // Now just collect the generators into Generator_System from the polynomial store
...
void Context::validate_generators(Generator_System & g){
   Generator_System g1=ps->minimized_generators(); // obtain the minimized generators from the polystore
...
void Context::validate_generators(Generator_System & g){
   ...
   for(vi=g1.begin();vi!=g1.end();vi++){
      ...
      if ((*vi).is_line()){
         // Convert generator (*vi is of line type) to Linear_Expression (no sign for line), 
         // then convert Linear_Expression to ray (signed with ray)
         Linear_Expression e;
...
void Context::obtain_primal_polyhedron(int left, C_Polyhedron & result){
   ...
   PRECONDITION( (result.space_dimension()== (unsigned) n) , " Polyhedron of wrong space dimension passed");
...
bool Context::obtain_transition_relation(int mult_index, int left, C_Polyhedron & result){
   ...
   //
   //
   // secondly all the constraints involving the multiplier should lie in the range [left.. left+n]
   ...
   //         -mc1 x1 - mc2 x2 ... -mcn xn - md + c1 x1' + ... + cn xn' +d <><> 0
...
bool Context::to_constraints_(int index, int left, C_Polyhedron & result, vector<Expression> * what, bool ineq){
