...
class SparseLinExpr{
  protected:
   ...
   //  4. count  (Added for the sake of maintaining consistency with an older implementation).  
   //
  ...
  public:
   ...
   int reset_count() {int t=count_; count_=0; return t; }
   ...
   // obtain the constraint correspondint to the ineq_type . see global_types.h for details
   ...
   // convert this into a lin expression, scale that by scale_fact and offset the variables by
   // adding offset. Add the result to ll
   
   void add_to_lin_expression(Linear_Expression & ll, int scale_fact=1, int offset=0) const;
   ...
   int obtain_largest_coefficient() const{
      if (is_zero()){
         cerr<<"Fatal Error in SparseLinExpr::obtain_largest_coefficient"<<endl << \
   ...
   void add_variables_in_front(int n1); // add n1 variables to the front of the expression
   
   void add_variables_in_back(int n1); // add n1 variables to the back of the expression
   
   void make_unprintable(){
      info_set_=false;
   }
...
ostream& operator<< (ostream & os, SparseLinExpr const & expr); // to print my beautiful Lin Exprs
